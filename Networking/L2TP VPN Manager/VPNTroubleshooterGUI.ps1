...
function Show-VPNTroubleshooterGUI {
            ...
            'Action' = {
                if ($vpnListBox.SelectedItem) {
                    try {
                        ...
                        # Try standard VPN connection first
                        try {
                            $vpnConfig = Get-VpnConnection -Name $vpnName -ErrorAction Stop
                        }
                        catch {
                            Update-Status "Standard VPN info not available, checking registry..."
                        }

                        # If standard method fails, check registry
                        if (-not $vpnConfig) {
                            $regPaths = @(
                            ...
                            foreach ($path in $regPaths) {
                                if (Test-Path $path) {
                                    ...
                                    if ($regInfo) {
                                        # Create custom object with available registry info
                                        $vpnConfig = [PSCustomObject]@{
                                            Name = $vpnName
                                            ServerAddress = $regInfo.PhoneNumber
                                            TunnelType = "L2TP"  # Assuming L2TP since that's our focus
                                            AuthenticationMethod = if ($regInfo.AuthenticationType) { $regInfo.AuthenticationType } else { "Unknown" }
                                            SplitTunneling = if ($regInfo.IpInterfaceMetric) { $true } else { $false }
                        ...
                        if ($vpnConfig) {
                            ...
                            if ($vpnConfig.PSObject.Properties.Match('Username').Count) {
                                $inputControls['Username'].Text = $vpnConfig.Username
                            }

                            # Set Authentication Type (if known)
                            if ($vpnConfig.PSObject.Properties.Match('L2tpPsk').Count) {
                                $inputControls['Authentication Type'].SelectedItem = 'PSK'
                                $inputControls['Pre-shared Key'].Text = $vpnConfig.L2tpPsk
                            } else {
                                $inputControls['Authentication Type'].SelectedItem = 'PSK'  # Default to PSK
                            }

                            # Populate Advanced Settings where available
                            if ($vpnConfig.PSObject.Properties.Match('SplitTunneling').Count) {
                                $inputControls['Split Tunneling'].Checked = $vpnConfig.SplitTunneling
                            }
                            if ($vpnConfig.PSObject.Properties.Match('RememberCredential').Count) {
                                $inputControls['Remember Credential'].Checked = $vpnConfig.RememberCredential
                            }
                            if ($vpnConfig.PSObject.Properties.Match('AuthenticationMethod').Count) {
                                $inputControls['Authentication Method'].SelectedItem = $vpnConfig.AuthenticationMethod
                            }
                            if ($vpnConfig.PSObject.Properties.Match('EncryptionLevel').Count) {
                                $inputControls['Encryption Level'].SelectedItem = $vpnConfig.EncryptionLevel
                            }
                            if ($vpnConfig.PSObject.Properties.Match('UseWinlogonCredential').Count) {
                                $inputControls['Use Winlogon Credential'].Checked = $vpnConfig.UseWinlogonCredential
                            }
                            if ($vpnConfig.PSObject.Properties.Match('IdleDisconnectSeconds').Count) {
                                $inputControls['Idle Disconnect (seconds)'].Text = $vpnConfig.IdleDisconnectSeconds
                            }
    ...
    @{
        ...
        'Action' = {
            if ($vpnListBox.SelectedItem) {
                $result = [System.Windows.Forms.MessageBox]::Show(
                    "Are you sure you want to delete the VPN connection '$($vpnListBox.SelectedItem)'?",
                    "Confirm Delete",
                    [System.Windows.Forms.MessageBoxButtons]::YesNo,
                    [System.Windows.Forms.MessageBoxIcon]::Warning
                )
                if ($result -eq 'Yes') {
                    Remove-VpnConnection -Name $vpnListBox.SelectedItem -Force
                    Update-Status "VPN connection '$($vpnListBox.SelectedItem)' deleted"
                    Update-VPNList
                }
            }
        }
...
$testButton.Add_Click({
    ...
    Update-Status "Testing VPN connection..."
    $results = Test-VPNConnection -VPNName $inputControls['VPN Name'].Text -ServerAddress $inputControls['Server Address'].Text
    if ($results) {
    ...
    @{
        ...
        'Action' = {
            ...
            $analyzeButton.Add_Click({
                ...
                if ($vpnName) {
                    Update-Status "Performing full analysis of VPN connection: $vpnName"
                    
                    # Check VPN Connection Status
                    $vpn = Get-VpnConnection -Name $vpnName
                    Update-Status "`nConnection Details:"
                    Update-Status "  Status: $($vpn.ConnectionStatus)"
                    Update-Status "  Server Address: $($vpn.ServerAddress)"
                    Update-Status "  Authentication Method: $($vpn.AuthenticationMethod)"
                    Update-Status "  Encryption Level: $($vpn.EncryptionLevel)"
                    Update-Status "  Split Tunneling: $($vpn.SplitTunneling)"
                    
                    # Test IPsec ports
                    Update-Status "`nTesting VPN Ports:"
                    $ports = @{
                        500 = 'IKE'
                        4500 = 'NAT-T'
                        1701 = 'L2TP'
                    ...
                    foreach ($port in $ports.Keys) {
                        ...
                        Update-Status "  Port $port ($($ports[$port])): $(if($test.TcpTestSucceeded){'Open'}else{'Closed'})"
                    }
                    
                    # Check Event Logs
                    Update-Status "`nChecking Event Logs:"
                    Get-WinEvent -FilterHashtable @{
                        LogName = 'Application','System'
                        StartTime = (Get-Date).AddHours(-1)
                        Keywords = 'RasClient','PPP','L2TP','IKE'
                    } -MaxEvents 10 -ErrorAction SilentlyContinue | ForEach-Object {
                        Update-Status "  $($_.TimeCreated) - $($_.Message)"
                    }

                    # Check Network Interface
                    Update-Status "`nChecking Network Interface:"
                    $vpnAdapter = Get-NetAdapter | Where-Object { $_.InterfaceDescription -like "*WAN Miniport (L2TP)*" }
                    if ($vpnAdapter) {
                        Update-Status "  Adapter Status: $($vpnAdapter.Status)"
                        Update-Status "  Speed: $($vpnAdapter.LinkSpeed)"
                        Update-Status "  Media State: $($vpnAdapter.MediaConnectionState)"
                    }

                    $analysisForm.Close()
                } else {
                    Update-Status "Please select a VPN connection to analyze"
                }
            })
